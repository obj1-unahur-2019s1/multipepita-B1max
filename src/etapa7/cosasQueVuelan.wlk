...
object helicopteroFeo {
	...
	method volar(km) {
		kmDesdeUltimoService += km
	}

	method requiereService() {
		return kmDesdeUltimoService >= frecuenciaServiceKm
	}

	method registarService() {
		kmDesdeUltimoService = 0
	}
...
object pepucha {
	...
	method comer(cosa, gramos) {
		cuantoComioDesdeUltimoVuelo  += cosa.energiaPorGramo() * gramos
	}

	method esBienTratada() {
		return kmVoladosConPilas >= kmVoladosSinPilas
	}
